import { MarketingPage } from "@/components/marketing/marketing-page";
import { MarketingWorkspace } from "@/components/marketing/marketing-workspace";
import { listMarketingResearch } from "@/modules/marketing/server/data";
import { requireEnabledPlugin } from "@/modules/plugins/server/guard";
export default async function Page({
  searchParams,
}: {
  searchParams: Promise<{ archived?: string }>;
}) {
  const { context } = await requireEnabledPlugin("marketing");
  const archived = (await searchParams).archived === "true";
  const records = await listMarketingResearch(
    context.organization.id,
    archived,
  );
  return (
    <MarketingPage
      activePath="/apps/marketing/research"
      context={context}
      description="Record manual research notes and source references."
      title="Research"
    >
      <MarketingWorkspace
        area="research"
        archived={archived}
        records={records}
        role={context.membership.role}
      />
    </MarketingPage>
  );
}
